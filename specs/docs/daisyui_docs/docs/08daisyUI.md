...
如何仅在页面的某个部分使用主题？
为任何元素添加 data-theme='THEME_NAME' 属性，其内部的所有内容都将应用该主题。 您可以嵌套主题，没有限制！ 您可以强制让 HTML 的某个部分只使用一个特定的主题。
index.html
...
如何为特定主题添加自定义样式？
您可以只为特定主题的元素编写自定义样式。 In this example, .my-btn class only will have this style on light theme.
app.css
[data-theme="light"] {
  .my-btn {
