...
def run_backtest(symbol: str, start_date: str, end_date: str,
    ...
    try:
        # 缩短查询范围，避免取到太远的日期（例如 API 返回了错误的大范围数据）
        end_dt_plus = (pd.to_datetime(end_date) + pd.Timedelta(days=10)).strftime('%Y%m%d')
    ...
    # 若在线获取失败或为空，尝试利用本地数据集中的未来日期（若存在）
    if next_open_after_end is None and not df.empty:
        try:
            end_dt_ts = pd.to_datetime(end_date)
            # df['date'] 已被转为 datetime 且排序
            future_mask = df['date'] > end_dt_ts
            if future_mask.any():
                # 取紧邻的下一天
                next_date_val = df.loc[future_mask, 'date'].iloc[0]
                next_open_after_end = next_date_val.strftime('%Y%m%d')
        except Exception:
            pass

    # 最后的兜底：若仍未找到，使用简单的日期推算（跳过周末）
    if next_open_after_end is None:
        try:
            d_curr = pd.to_datetime(end_date)
            # 往后找 1-5 天，找到第一个周一到周五
            for i in range(1, 6):
                candidate = d_curr + pd.Timedelta(days=i)
                if candidate.weekday() < 5: # 0-4 是周一到周五
                    next_open_after_end = candidate.strftime('%Y%m%d')
                    break
        except Exception:
            pass
