...
export default function BacktestClient({ initialCredits }: BacktestClientProps) {
  ...
  const [sessionId, setSessionId] = useState<string | null>(null); // New: Session ID
  ...
  const handleStrategyApply = async (config: any) => {
    // 1. Create Session & Deduct Credits (if logged in and starting fresh)
    // Only create a NEW session if we don't have one yet (sessionId is null)
    // This supports the flow: New -> Apply (Create Session A) -> Chat More -> Apply (Update Session A)
    // If user clicks "Create New" again, sessionId is reset to null, so next Apply creates Session B.
    if (session?.user && !sessionId) {
        try {
            const res = await fetch('/api/backtest/session/create', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    name: userIntent.slice(0, 50) || 'New AI Strategy',
                    symbol: symbol,
            ...
            if (res.ok && data.sessionId) {
                ...
                // Silent success: Don't show toast for session creation to reduce noise
            } else {
                showToast(data.error || 'Failed to create session record', 'error');
            }
        } catch (e) {
  ...
  const handleCreateNew = () => {
    setChatHistory([]); // Start fresh
    setChatMode('creation');
    setSessionId(null); // Reset session ID so next Apply creates a new record
  ...
  const handleEdit = () => {
    // Do NOT clear chat history here, preserve context for editing
    setChatMode('edit'); 
    setDiagnosisContext(null); 
    setIsWizardOpen(true);
  };
  ...
  const runBacktest = async () => {
    ...
    try {
      ...
      if (finalResult) {
        ...
        // --- NEW: Save Run Record to Supabase (if Session exists) ---
        if (session?.user && sessionId) {
            try {
                ...
                await supabase.from('backtest_runs').insert({
                    ...
                    user_id: session.user.uuid, // Use stored UUID
                    strategy_config: strategyConfig,
                    start_date: startDate,
                    end_date: endDate,
                    stock_code: symbol, // Save the actual symbol used for this run
                    initial_cash: 100000, // Hardcoded for now in demo
                    final_equity: finalResult.final_equity,
                    return_pct: ((finalResult.final_equity - 100000) / 1000).toFixed(2),
                    win_rate: finalResult.report?.win_rate || 0,
                    total_trades: finalResult.report?.total_trades || 0,
                    result_summary: finalResult.report
                });
                
                // Optionally update Session updated_at (handled by trigger, but we could do it here too)
            } catch (err) {
